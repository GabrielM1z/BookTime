...
func (br *BookRepository) SelectGenresByBookID(bookID string) []model.Genre {
	query := `
...
func (br *BookRepository) InsertBook(book model.Book) bool {
	_, err := br.DB.Exec("INSERT INTO book (id_book, title, description, id_format, publisher, publication_date, page_number, language, cover_image_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
		book.IdBook, book.Title, book.Description, "BOOK", book.Publisher, book.PublicationDate, book.PageNumber, book.Language, book.CoverImageUrl)
	if err != nil {
		log.Println(err)
		return false
	}
...
func (br *BookRepository) SelectBook(id string) (*model.Book, error) {
	query := `
        SELECT id_book, title, description, id_format, publisher, 
               TO_CHAR(publication_date,'YYYY-MM-DD') as publication_date, page_number, language, cover_image_url
...
func (br *BookRepository) UpdateBook(book model.Book) bool {
...
func (br *BookRepository) DeleteBook(id string) bool {
